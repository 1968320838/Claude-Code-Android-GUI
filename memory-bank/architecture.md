...
│           └── ConnectionState.java # 连接状态（抽象类层次）
├── res/
...
| 文件 | 作用 |
|------|------|
| `settings.gradle.kts` | 项目配置，定义仓库和模块包含 |
| `build.gradle.kts` (根) | 全局插件版本，AGP 8.2.2, Kotlin 1.9.22, Hilt 2.48.1 |
| `gradle.properties` | Gradle 属性，AndroidX 启用，Jetifier |
| `app/build.gradle.kts` | 模块依赖声明：Material, Navigation, Room, Hilt, Coroutines, JSch, Markwon |
...
| `TermuxRepository` | SSH 操作接口：connect, disconnect, executeCommand, isConnected |
...
| `SettingsFragment/ViewModel` | 设置界面，HiltViewModel 注入 |
...
| `colors.xml` | 完整 Material You 色彩系统定义 |
...
使用 **Dagger Hilt**，所有 @HiltAndroidApp 的 Application 类，所有 ViewModel 使用 @HiltViewModel 注解。

## 后续扩展

- 添加 `data/local/` - Room 数据库和 DAO
- 添加 `data/repository/` - Repository 实现类
- 添加 `di/` - Hilt Module
- `assets/` - xterm.js 终端模拟器资源
